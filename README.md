...
> ⚠️ **미검증 서비스**: 네이트 IMAP 연결은 충분히 테스트되지 않았습니다. 동작 결과를 [Issues](https://github.com/youngsooco/k-mail-mcp/issues)에 공유해 주세요.
...
> ⚠️ **미검증 서비스**: Yahoo IMAP 연결은 충분히 테스트되지 않았습니다. 동작 결과를 [Issues](https://github.com/youngsooco/k-mail-mcp/issues)에 공유해 주세요.
...
> ⚠️ **미검증 서비스**: iCloud IMAP 연결은 충분히 테스트되지 않았습니다. 동작 결과를 [Issues](https://github.com/youngsooco/k-mail-mcp/issues)에 공유해 주세요.
...
A. 다음 IMAP 서버는 폴더명 대소문자를 구분합니다. `list_mailboxes` 툴로 실제 폴더명을 확인한 뒤 정확한 이름을 사용해주세요. (예: `INBOX`가 아닌 `Inbox`일 수 있습니다.)

**Q. `watched_mailboxes.json`이 자꾸 git에 올라갑니다.**
A. 이 파일은 이 PC 전용 폴더 캐시로, git에 포함되면 안 됩니다. v1.3.0부터 `.gitignore`에 자동 포함됩니다. 이전 버전에서 업그레이드했다면 수동으로 `.gitignore`에 `watched_mailboxes.json`을 추가하세요.
