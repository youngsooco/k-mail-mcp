...
- 각 메일 서비스의 **계정 비밀번호**
  > 💡 대부분의 경우 평소 로그인 비밀번호를 그대로 입력하면 돼요.
  > ⚠️ **2단계 인증(2FA) 사용 중이라면**: 로그인 비밀번호 대신 보안 설정에서 별도 발급한 **앱 비밀번호**를 입력해야 해요. 아래 발급 방법 참고
...
대부분의 경우 **평소 로그인 비밀번호를 그대로 입력**하면 돼요.

**2단계 인증(2FA) 사용 중인 경우에만** 아래 절차가 필요해요:
각 서비스 보안 설정에서 **앱 비밀번호**를 별도로 발급받아 입력해야 해요.
로그인 비밀번호로는 IMAP 연결이 거부됩니다.

> 💡 2단계 인증을 사용하지 않는 경우 아래 절차 없이 로그인 비밀번호를 그대로 입력하면 됩니다.
...
2. `2단계 인증` 활성화 (필수)
3. `앱 비밀번호` 검색 → 앱 선택 후 생성 (이것이 계정 비밀번호)
4. 생성된 16자리 비밀번호 복사 → setup.bat (Windows) 또는 setup.sh (macOS)에서 입력
