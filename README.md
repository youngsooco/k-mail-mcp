...
A. Windows Store(MSIX) 방식으로 설치된 Claude Desktop은 config 파일이 두 곳에 존재합니다. v1.0.2부터 `install.bat`이 MSIX 경로를 자동 탐색해 올바른 파일에 기록합니다. `install.bat`을 다시 실행 후 Claude Desktop을 트레이에서 완전 종료 → 재시작하세요.
