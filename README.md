...
[![Version](https://img.shields.io/badge/version-1.4.0-brightgreen.svg)](https://github.com/youngsooco/k-mail-mcp/releases)
...
> 버전에 따라 MCP 연결 방식과 지원 클라이언트가 다릅니다. 사용 목적에 맞는 버전을 확인하세요.

| 버전 | 연결 방식 | 대상 클라이언트 | 인증 | 주요 변경 |
|------|----------|--------------|------|---------|
| **v1.0–1.1** | stdio (로컬) | Claude Desktop 전용 | 없음 (로컬 프로세스) | 기본 메일 수집·스팸 탐지 |
| **v1.2** | stdio (로컬) | Claude Desktop | 없음 | 프롬프트 인젝션 방어, AES-256-GCM, TLS 강제 |
| **v1.3** | stdio (로컬) + HTTP 선택 | Claude Desktop, Claude Code | Bearer 토큰 (HTTP 모드) | 전체 폴더 동적 순회, watched_mailboxes |
| **v1.4** | stdio (로컬) + **HTTP + OAuth 2.0** | **Claude Desktop, Claude Code, claude.ai (원격)** | OAuth 2.0 + PKCE | claude.ai custom connector 완전 지원 |

### 어떤 버전을 써야 하나요?

```
Claude Desktop(로컬)만 사용 → 어떤 버전이든 stdio 모드로 동일하게 동작
claude.ai 웹/앱에서 사용   → v1.4.0 필수 (OAuth 2.0 원격 서버 모드)
외부 서버 배포             → v1.4.0 (HTTP + OAuth2 모드, MAIL_MCP_HTTP_PORT 설정)
```
...
| 서비스 | 이메일 도메인 | IMAP 주소 | 검증 상태 | 비고 |
|--------|-------------|-----------|----------|------|
| 네이버 | @naver.com | imap.naver.com | ✅ 검증완료 | 국내 1위 |
| 다음/카카오 | @daum.net @kakao.com | imap.daum.net | ✅ 검증완료 | 국내 2위 |
| Gmail | @gmail.com | imap.gmail.com | ✅ 검증완료 | 글로벌 1위 |
| 네이트 | @nate.com | imap.nate.com | ⚠️ 미검증 | 국내 SK |
| Yahoo | @yahoo.com | imap.mail.yahoo.com | ⚠️ 미검증 | 글로벌 |
| iCloud | @icloud.com @me.com | imap.mail.me.com | ⚠️ 미검증 | Apple, 앱 전용 암호 필수 |

> ⚠️ **미검증 서비스 사용자 분들께:** 네이트·Yahoo·iCloud는 IMAP 프로토콜 설정을 기준으로 구현했으나 실제 환경에서 충분히 테스트되지 않았습니다.
> 동작 여부(성공·실패·오류 메시지)를 [GitHub Issues](https://github.com/youngsooco/k-mail-mcp/issues)에 알려주시면 빠르게 반영하겠습니다. Community PRs are always welcome!
...
| 클라이언트 | 지원 여부 | MCP 방식 | 필요 버전 | 비고 |
|---|---|---|---|---|
| **Claude Desktop** | ✅ 완전 지원 | 로컬 stdio | v1.0+ | 커스텀 출력 포맷, 전체 기능 |
| Claude Code | ✅ 지원 가능 | stdio | v1.0+ | `claude mcp add` 명령으로 연결 |
| **claude.ai (웹/앱)** | ✅ **v1.4.0 완전 지원** | 원격 HTTP + OAuth 2.0 | **v1.4.0** | 원격 서버 배포 필요 → [설정 가이드](#claudeai-원격-mcp-연결-v140) |
...
- [Claude Desktop](https://claude.ai/download) 설치 (Claude Desktop 사용 시)
...
## claude.ai 원격 MCP 연결 (v1.4.0)

> **이 섹션은 claude.ai 웹/앱에서 직접 K-Mail-MCP를 사용하고 싶을 때 필요합니다.**
> Claude Desktop만 사용하는 경우 이 섹션은 건너뛰어도 됩니다.

v1.4.0부터 **OAuth 2.0** 인증을 지원합니다. claude.ai의 원격 MCP 서버 기능을 사용해 네이버·다음·Gmail을 직접 연결할 수 있습니다.

### 작동 원리

```
claude.ai → GET /.well-known/oauth-authorization-server   # OAuth 서버 자동 탐색
claude.ai → POST /register                                 # 동적 클라이언트 등록 (자동)
브라우저  → GET /authorize → 로그인 페이지 표시
사용자    → API 키 입력 → POST /oauth/submit-key
서버      → 302 redirect (auth code)
claude.ai → POST /token                                    # access_token 발급 (24h)
claude.ai → POST /mcp + Bearer token                       # MCP 연결 시작
```

### 사전 요구사항

- 외부에서 접근 가능한 서버 (공인 IP 또는 Cloudflare Tunnel 등)
- Node.js v20+ 설치
- `MCP_API_KEY` 환경변수 설정 (이 값이 로그인 비밀번호 역할)
...
export MCP_API_KEY="your-secret-api-key"          # 로그인 시 입력할 비밀번호
export MAIL_MCP_BASE_URL="https://your-domain.com" # 공개 접속 URL (trailing slash 제외)

# 서버 시작
node index.js
```

시작 시 아래 로그가 출력되면 정상입니다:
```
[k-mail-mcp] v1.4.0 OAuth2 MCP 서버 시작 — port 8766
...
1. claude.ai → **설정(Settings)** → **연결(Connections)** 탭
2. **MCP 서버 추가(Add MCP Server)** 클릭
3. 아래 값 입력:

| 항목 | 값 | 설명 |
|------|-----|------|
| **이름(Name)** | `k-mail-mcp` | 원하는 이름으로 설정 |
| **URL** | `https://your-domain.com/mcp` | `/mcp` 경로 필수 |
| **OAuth 클라이언트 ID** | *(비워두기)* | 자동 동적 등록 사용 |
| **OAuth 클라이언트 시크릿** | *(비워두기)* | Public client (PKCE 전용) |

> **OAuth 클라이언트 ID / 시크릿 필드에 대해**
>
> claude.ai의 고급 설정에서 OAuth 클라이언트 ID와 시크릿을 입력하는 필드가 있습니다.
> K-Mail-MCP는 **동적 클라이언트 등록(Dynamic Client Registration, RFC 7591)**을 지원하므로 이 필드는 **비워두세요**.
> claude.ai가 최초 접속 시 `/register` 엔드포인트를 통해 자동으로 클라이언트를 등록합니다.
> 별도로 발급한 OAuth 클라이언트 ID가 있는 경우(직접 `/register`를 호출해 사전 등록한 경우)에만 입력하세요.

4. **저장** 후 **연결(Connect)** 클릭
5. 브라우저에서 K-Mail-MCP 로그인 페이지가 열립니다
6. **`MCP_API_KEY`로 설정한 값**을 API 키 필드에 입력 후 **연결 허용** 클릭
7. 인증 완료 — claude.ai 창으로 돌아와 k-mail-mcp 툴이 활성화됩니다

### 토큰 만료 및 재인증

- 액세스 토큰 유효 기간: **24시간**
- 서버 재시작 시 모든 토큰 초기화 (재로그인 필요)
- 토큰 만료 시 claude.ai가 자동으로 재인증 페이지를 표시합니다

### Cloudflare Tunnel로 배포하기 (권장)

고정 IP 없이 로컬 서버를 외부에 안전하게 노출할 수 있습니다:

```bash
# Cloudflare Tunnel 설치 (cloudflared)
# https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/

# 터널 연결 (예시)
cloudflared tunnel --url http://localhost:8766

# 출력된 URL을 MAIL_MCP_BASE_URL에 설정
export MAIL_MCP_BASE_URL="https://xxxx.trycloudflare.com"
```

> CF Tunnel 무료 플랜은 커스텀 도메인 없이도 사용 가능합니다.
> 단, 랜덤 URL이 매번 변경되므로 고정 도메인이 필요한 경우 Cloudflare 계정 연결 후 커스텀 도메인을 설정하세요.
...
> ⚠️ **미검증 서비스**: 네이트 IMAP 연결은 충분히 테스트되지 않았습니다.
...
> ⚠️ **미검증 서비스**: Yahoo IMAP 연결은 충분히 테스트되지 않았습니다.
...
> ⚠️ **미검증 서비스**: iCloud IMAP 연결은 충분히 테스트되지 않았습니다.
...
Claude에서 요청하면 실제 메일 패턴을 분석해 카테고리를 만들어줍니다:
```
"내 메일 패턴 보고 카테고리 자동으로 만들어줘"
...
| auth code TTL | 5분 — 단기 유효, 재사용 불가 |
| access token TTL | 24시간 — 만료 시 재인증 |
| 토큰 저장 | 서버 인메모리 — 파일·DB 미기록 |
| 동적 클라이언트 등록 | 지원 (RFC 7591) — 사전 등록 불필요 |
...
| 버전 | 내용 |
|------|------|
| v1.2.1 | 프롬프트 인젝션 방어(`sanitizeForLLM`), TLS 강제(`rejectUnauthorized: true`), atomic write, 파일 권한 0o600 적용 |
| v1.3.0 | GitHub Actions 워크플로우 hardening (concurrency, timeout, fork guard) |
| v1.4.0 | OAuth 2.0 도입, `timingSafeEqual` API 키 비교, Open Redirect 방어, PKCE S256 강제 |
...
```
k-mail-mcp/
├── index.js             MCP 서버 본체 (v1.4.0) — stdio / HTTP+OAuth2 듀얼 모드
├── oauth.js             OAuth 2.0 Provider 구현 (v1.4.0 신규)
...
### claude.ai 원격 MCP (OAuth 2.0 모드)

**Q. claude.ai에서 연결 시도 시 "MCP 서버를 찾을 수 없습니다" 오류가 납니다.**
A. `MAIL_MCP_BASE_URL`이 외부에서 실제로 접근 가능한 URL인지 확인하세요. 브라우저에서 `https://your-domain.com/.well-known/oauth-authorization-server`를 직접 열어 JSON이 반환되면 정상입니다.

**Q. OAuth 클라이언트 ID / 시크릿에 뭘 입력해야 하나요?**
A. **비워두세요.** K-Mail-MCP는 동적 클라이언트 등록(RFC 7591)을 지원합니다. claude.ai가 최초 접속 시 자동으로 등록합니다. 직접 `/register` 엔드포인트를 호출해 사전 발급한 `client_id`가 있는 경우에만 입력하세요.

**Q. 서버 재시작 후 claude.ai에서 다시 로그인하라고 합니다.**
A. 정상 동작입니다. 액세스 토큰이 인메모리에만 저장되므로 서버 재시작 시 초기화됩니다. 24시간마다도 재인증이 필요합니다.

**Q. API 키는 무엇을 입력해야 하나요?**
A. 서버 시작 시 설정한 `MCP_API_KEY` 환경변수 값입니다. 이 값이 OAuth 로그인 페이지의 "API 키" 필드에 입력할 비밀번호입니다.

**Q. 로컬 PC에서 서버를 실행하고 싶은데 외부 접근이 안 됩니다.**
A. Cloudflare Tunnel(무료)을 사용하면 고정 IP 없이도 외부 노출이 가능합니다. 단, 랜덤 URL은 재시작마다 변경되므로 고정 URL이 필요하면 Cloudflare 계정 연결 후 커스텀 도메인을 설정하세요.
...
**Q. 다음 메일에서 특정 폴더가 인식되지 않습니다.**
A. 다음 IMAP 서버는 폴더명 대소문자를 구분합니다. `list_mailboxes` 툴로 실제 폴더명을 확인한 뒤 정확한 이름을 사용해주세요.

**Q. `watched_mailboxes.json`이 자꾸 git에 올라갑니다.**
A. 이 파일은 이 PC 전용 폴더 캐시로, git에 포함되면 안 됩니다. v1.3.0부터 `.gitignore`에 자동 포함됩니다.
...
A. Claude에서 "내 메일 패턴 보고 카테고리 자동으로 만들어줘"라고 요청하면 `generate_categories` 툴이 실행됩니다. 또는 프로젝트 폴더의 `categories.json`을 직접 편집하세요.

**Q. 특정 폴더를 스캔에서 빼고 싶어요.**
A. Claude에서 "○○ 폴더 스캔에서 제외해줘"라고 요청하면 `set_watched_mailboxes` 툴이 실행됩니다.
