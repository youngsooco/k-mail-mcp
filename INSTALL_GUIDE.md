...
2. **LTS** 버전 다운로드 (**v20 이상 권장** (v22 LTS 권장 / 최소 v18 이상))
...
> ℹ️ **Windows MSIX 설치판**: `install.bat` v1.0.2부터 MSIX 경로 자동 감지. 문제가 있으면 설정 → 개발자 → k-mail-mcp → 로그 보기로 원인 확인 가능.
...
> 앱 이름은 식별용 명칭으로 아무 이름이나 입력해도 됩니다. (예: "Claude", "K-Mail")
...
- `node --version`으로 버전 확인 (v20 이상 권장, 최소 v18)
